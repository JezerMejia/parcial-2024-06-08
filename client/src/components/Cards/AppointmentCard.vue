...
<template>
    <li class="flex flex-col  bg-white">
        ...
        <ExcecutionState class="border-x border-blue-300 text-sm" :state="props.appointmentCardType.status"/>
        <!--Fechas-->
        <div class="flex flex-1 flex-row border-x border-blue-300 p-4">
            <ul class="flex w-full flex-col justify-center gap-2 text-blue-500">
                <li class="flex items-center gap-2">
                    <VueFeather class="size-10 min-w-10" type="calendar" stroke="#01193F" stroke-width="1.5"></VueFeather>
                    <div class="flex flex-col">
                        <p class="text-xs font-semibold">Fecha de inicio</p>
                        <p class="text-base font-semibold">{{ getFormattedDateTime(props.appointmentCardType.startDate) }}</p>
                    </div>
                </li>
                <li class="flex items-center gap-2">
                    <VueFeather class="size-10 min-w-10" type="calendar" stroke="#01193F" stroke-width="1.5"></VueFeather>
