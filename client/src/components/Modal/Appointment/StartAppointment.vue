...
import { startAppointment } from "@/composables/useAppointment";
...
async function request(): Promise<boolean> {
  const { data } = await startAppointment(props.uuid);
...
<template>
  <Modal ref="modal">
    ...
    <form novalidate class="flex flex-col gap-0" @submit.prevent="handleSubmit">
      <div class="flex flex-col gap-2.5 p-4">¿Desea iniciar la cita médica?</div>
